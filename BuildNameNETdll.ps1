# Name:    BuildNameNETdll
# Purpose: Compile the mdNameNET.dll wrapper
...
Write-Host "`n======================= WELCOME TO MELISSA NAME OBJECT NET WRAPPER =======================`n"
...
cmd.exe /C """$CmdPath"" x86_x64 && csc -out:mdNameNET.dll -platform:AnyCPU -target:library mdName_cSharpCode.cs" > $null

if (Test-Path ("mdNameNET.dll")) {
  $filePath = Join-Path -Path (Split-Path -Parent $PSCommandPath) -ChildPath "mdNameNET.dll"

  Write-Host "`nmdNameNET.dll has been successfully generated at:`n$filePath`n"
}
else {
  Write-Host "`nError: mdNameNET.dll was not created successfully`n"
